...
class EmotionObject{
  ...
  getOnomatopoeiaWords(){
    let words = [];
    for(let i = 0; i < this.onomatopoeia.lemgth; i++){
      words = new Word(this.onomatopoeia, dictionary[this.onomatopoeia[i]], pictureDictionary(this.onomatopoeia[i]))
    }
    console.log(words);
    return words;
  }
...
// 次にこの感情の各擬音語とその定義、画像を含んだカードが表示されます。
  getHtmlContainerString(i){
    let words = this.getOnomatopoeiaWords();
    let containerSection = "<div>";
      containerSection +=
      `
          <div id="sec${i}" class="bg-${this.color} big-square">
              <h2>${this.emotion}</h2>
      ...
      for(let i = 0; i < words.length; i++){
        ...
        `
        <div class="bg-white big-square">
            <h2>${words[i].defintion}</h2>
            <p>${words[i].pictureUrl}</p>
        </div>
        `
        containerSection += "</div>";
      }
...
for(let i = 0; i < emotions.length; i++){
   containerSection += emotions[i].getHtmlContainerString(i);
}
